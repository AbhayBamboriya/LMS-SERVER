...
const addingDocs = async(req,res,next)=>{
    try{
        const {description}=req.body
        // const {id}=req.params;
        // console.log("idddd "+id);
        const { courseId, lectureId } = req.query;
        if(!courseId || !lectureId || !description){
            return next(
                new AppError('All fields are required',400)
            )
        }
        console.log("doc1",courseId);
        const course=await Course.findById(courseId)
        if(!course){
            return next(
        ...
        if(req.file){
            try{
                ...
                if(result){
                    lectureData.docs.public_id=result.public_id 
                    console.log('result secure',result.secure_url);
                    lectureData.docs.secure_url=result.secure_url
                }
